...
commits the result. That last push uses the `INDEX_DEPLOY_KEY` deploy key, since `main`
requires the validate check and the workflow's own token cannot get past it.

The docs page for a template embeds that thread through giscus, so reactions and replies live
on GitHub.
